...
def wait_for_http_inside_pod(
    ...
    probe_script = (
        ...
        f"response = urllib.request.urlopen({probe_url!r}, timeout=2)\n"
    ...
    probe_command = [
        "sh",
        "-c",
        f"python -c {shlex.quote(probe_script)} || python3 -c {shlex.quote(probe_script)}",
    ]
    ...
    try:
        for _ in TimeoutSampler(
            ...
            sleep=5,
            func=lambda: pod.execute(container=container_name, command=probe_command, timeout=10),
...
def build_dashboard_image_patch(
    ...
    if resolved_image.build_commit:
        patches.append({
            "op": (
                "replace"
                if annotations.get("notebooks.opendatahub.io/last-image-version-git-commit-selection")
                else "add"
            ),
