...
uv run pytest --pre-upgrade tests/workbenches/notebook_images/ --tc workbench_upgrade_track=stable
```
...
- On clusters where RStudio images are built in-cluster, the test can trigger `rstudio-server-rhel9` BuildConfig when the tag is not yet imported.
